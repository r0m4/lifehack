...
MW = require('../libs/multiwait'),
Transformer = require('../libs/transformer'),
...
Tests.prototype.testsimple = function() {
	var self = this
	var input = 'input1'
	this.transformer.applyTemplate(input).once('ready', function(output) {
		self.assertEquals('testsimple.1', output, 'input1')

		input = 'input2 {abc}'
		self.transformer.applyTemplate(input).once('ready', function(output) {
			self.assertEquals('testsimple.2', output, 'input2 {abc}')

			input = 'input3'
			self.transformer.applyTemplate(input).once('ready', function(output) {
				self.assertEquals('testsimple.3', output, 'input3')
...
Tests.prototype.testvars = function() {
	...
	var input = 'abcd {abc\n {$ a}\n} {$ b} {$ c} {$ a} {$ n*4 - n/2}'
	var self = this
	self.transformer.applyTemplate(input, vars, null).once('ready', function(output) {
		var verify = 'abcd {abc\n A\n} BBB C A 28'
		self.assertEquals('testvars.1', output, verify, 'output', 'verify')
		...
		ins = self.transformer.compile(input)
		self.transformer.execute(ins, vars).once('ready', function(output) {
			self.assertEquals('testvars.unknown_macro', output, verify, 'output', 'verify')
		
			self.transformer.ignoreUnknownMacros = true
			self.transformer.execute(ins, vars).once('ready', function(output) {
				self.assertEquals('testvars.ignore_unknown_macro', output, verify2, 'output', 'verify2')
				self.transformer.ignoreUnknownMacros = false
				...
				var input = "Some new data {$var1.field1[2]} and {$var2()} with  some {$var1.field1[1].name} with unmatched {$var3} with unmatched {$var3.xyz}"
				var verify = "Some new data V1_F1_3 and Hurray with  some V1_F1_name with unmatched  with unmatched "
				var ins = self.transformer.compile(input)
				self.transformer.execute(ins, vars).once('ready', function(output) {
					self.assertEquals('testvars.3', output, verify, 'output', 'verify')
					self.next()
				})
			})
		})
...
Tests.prototype.testmacro = function() {
	...
	this.transformer.execute(ins, vars).once('ready', function(output) {
		self.assertEquals('testmacro.basic', output, verify, 'output', 'verify')
		
		// Macro with params
		self.transformer.addMacro('simple_macro2', function(context) {
		...
		ins = self.transformer.compile(input)
		self.transformer.execute(ins, vars).once('ready', function(output) {
			self.assertEquals('testmacro.params', output, verify, 'output', 'verify')

			// Macro with vars for fields with undefined vars
			self.transformer.addMacro('simple_macro3', function(context) {
			...
			ins = self.transformer.compile(input)
			self.transformer.execute(ins, vars).once('ready', function(output) {
				self.assertEquals('testmacro.paramvars', output, verify, 'output', 'verify')
				...
				ins = self.transformer.compile(input)
				self.transformer.execute(ins, vars).once('ready', function(output) {
					self.assertEquals('testmacro.undefined', output, verify, 'output', 'verify')
...
Tests.prototype.testhtml =function() {
	...
	transformer.execute(instructions, vars).once('ready', function(processed) {
		self.assertEquals('testhtml', processed, verify, 'processed', 'verify')
		self.next()
	})
}
...
Tests.prototype.testpersistance =function() {
	...
	var vars = {title: "Template Test", defaults: {age: 40}}
	transformer2.execute(loadback, vars).once('ready', function(output) {
		self.assertEquals('testpersistance', output, verify, 'output', 'verify')
		self.next()
	})
}
...
Tests.prototype.testblocks =function() {
	...
	transformer.execute(ins, vars).once('ready', function(output) {
		self.assertEquals('testblock.basic', output, verify, 'output', 'verify')
		self.next()
	})
}
...
Tests.prototype.testfilter = function() {
	...
	this.transformer.execute(code, {name: 'james Bond'}).once('ready', function(output) {
		self.assertEquals('testspecial.filter', output, verify, 'output', 'verify')
		
		// userdefined
		var myfilter = function(a) { return a.length }
		self.transformer.addFilter('upper', myfilter)
		self.transformer.execute(code, {name: 'james Bond'}).once('ready', function(output) {
			var verify = 'The name is 10'
			self.assertEquals('testspecial.filter2', output, verify, 'output', 'verify')
...
Tests.prototype.testspecial_0 = function() {
	var self = this
	// simple if
	var input = '{if var1 > var2}All is well{/if}'
	//var input = '{loop count=3}abcd{/loop}'
	var verify = 'All is well'
	var code = this.transformer.compile(input)
	this.transformer.execute(code, {var1: 3, var2: 1}).once('ready', function(output) {
		self.assertEquals('testspecial0.if1', output, verify, 'output', 'verify')
		self.next()
	})
}

Tests.prototype.testspecial_1 = function() {
	var self = this
	// {set key1=value1}
	...
	var code = this.transformer.compile(input)
	this.transformer.execute(code).once('ready', function(output) {
		self.assertEquals('testspecial.set', output, verify, 'output', 'verify')
		self.next()
	})
}
...
Tests.prototype.testspecial_2 = function() {
	var self = this
	// simple if
	var input = 'Prefix {if var1 < var2}Well {=var1} is less than {=var2}{/if}{if var1 == var2}Well {=var1} is equal to {=var2}{/if}{if var1 > var2}Well {=var1} is greater than {=var2}{/if} Suffix'
	var verify1 = 'Prefix Well 3 is greater than 1 Suffix'
	var verify2 = 'Prefix Well 3 is less than 5 Suffix'
	var verify3 = 'Prefix Well 3 is equal to 3 Suffix'
	
	code = this.transformer.compile(input)
	this.transformer.execute(code, {var1: 3, var2: 1}).once('ready', function(output) {
		self.assertEquals('testspecial.if1', output, verify1, 'output', 'verify1')

		self.transformer.execute(code, {var1: 3, var2: 5}).once('ready', function(output) {
			self.assertEquals('testspecial.if2', output, verify2, 'output', 'verify2')
			self.transformer.execute(code, {var1: 3, var2: 3}).once('ready', function(output) {
				self.assertEquals('testspecial.if3', output, verify3, 'output', 'verify3')

				// simple if/else
				input = 'Prefix {if var1 < var2}LESS{else}GREATER{/if} Suffix'
				verify1 = 'Prefix LESS Suffix'
				verify2 = 'Prefix GREATER Suffix'
				code = self.transformer.compile(input)
				self.transformer.execute(code, {var1: 3, var2: 1}).once('ready', function(output) {
					self.assertEquals('testspecial.ifelse1', output, verify2, 'output', 'verify2')

					self.transformer.execute(code, {var1: 1, var2: 3}).once('ready', function(output) {
						self.assertEquals('testspecial.ifelse1', output, verify1, 'output', 'verify1')
						self.next()
					})
				})
			})
...
Tests.prototype.testspecial_3 = function() {
	var self = this
	// Nested if
	...
	code = this.transformer.compile(input)
	this.transformer.execute(code, {var1: 1, var2: 10}).once('ready', function(output) {
		self.assertEquals('testspecial.nested_if1', output, verify, 'output', 'verify')
		self.next()
	})
}

Tests.prototype.testspecial_4 = function() {
	var self = this
	// simple while
	...
	code = this.transformer.compile(input)
	this.transformer.execute(code).once('ready', function(output) {
		self.assertEquals('testspecial.while', output, verify, 'output', 'verify')
		self.next()
	})
}


Tests.prototype.testspecial_5 = function() {
	var self = this
	// while with sets
	...
	code = this.transformer.compile(input)
	this.transformer.execute(code, {var1: 1}).once('ready', function(output) {
		self.assertEquals('testspecial.while2', output, verify, 'output', 'verify')
		self.next()
	})
}

Tests.prototype.testspecial_6 = function() {
	var self = this
	// simple foreach
	...
	code = this.transformer.compile(input)
	this.transformer.execute(code, {purchase: purchase}).once('ready', function(output) {
		self.assertEquals('testspecial.foreach1', output, verify, 'output', 'verify')
	// foreach with empty data
		self.transformer.execute(code, {orders: []}).once('ready', function(output) {
			self.assertEquals('testspecial.foreach2', output, verify2, 'output', 'verify2')
			// foreach with undefined var
			self.transformer.execute(code, {}).once('ready', function(output) {
				self.assertEquals('testspecial.foreach3', output, verify2, 'output', 'verify2')
				self.next()
			})
		})
	})
...
Tests.prototype.testspecial_7 = function() {
	var self = this
	// foreachelse
	...
	code = this.transformer.compile(input)
	output = this.transformer.execute(code, {orders: orders}).once('ready', function(output) {
		self.assertEquals('testspecial.foreachelse', output, verify, 'output', 'verify')
		// foreachelse with empty data
		output = self.transformer.execute(code, {orders: []}).once('ready', function(output) {
			self.assertEquals('testspecial.foreachelse2', output, verify2, 'output', 'verify2')
			// foreachelse with undefined var
			output = self.transformer.execute(code, {}).once('ready', function(output) {
				self.assertEquals('testspecial.foreachelse3', output, verify2, 'output', 'verify2')
				self.next()
			})
		})
	})
...
Tests.prototype.testspecial_8 = function() {
	var self = this
	// ldelim and rdelim
	...
	code = this.transformer.compile(input)
	this.transformer.execute(code, {orders: orders}).once('ready', function(output) {
		self.assertEquals('testspecial.delims1', output, verify, 'output', 'verify')
		self.next()
	})
}


Tests.prototype.testspecial_9 = function() {
	var self = this
	// escaping macro
	...
	code = this.transformer.compile(input)
	this.transformer.execute(code, {orders: orders}).once('ready', function(output) {
		self.assertEquals('testspecial.delims2', output, verify, 'output', 'verify')
		self.next()
	})
}
...
Tests.prototype.testspecial2 = function() {
	...
	var code = this.transformer.compile(input)
	this.transformer.execute(code, vars).once('ready', function(output) {
		self.assertEquals('testspecial2.capture1', output, verify, 'output', 'verify')
		self.assertEquals('testspecial2.capture1var', vars.somevar1, verify2, 'vars.somevar1', 'verify2')
		self.next()
	})
}
...
Tests.prototype.test_extensions = function() {
	...
	transformer1.execute(transformer1.compile(input)).once('ready', function(output) {
		output1 = output
		mw.finishPending()
	})

	transformer2.execute(transformer2.compile(input)).once('ready', function(output) {
		output2 = output
		mw.finishPending()
	})

	transformer3.execute(transformer3.compile(input)).once('ready', function(output) {
		output3 = output
		mw.finishPending()
	})

	mw.waitall(function() {
		self.assertEquals('test_extensions.basic', output1, verify1, 'output1', 'verify11')
		self.assertEquals('test_extensions.namespace1', output2, verify2, 'output2', 'verify12')
		self.assertEquals('test_extensions.namespace2', output3, verify3, 'output3', 'verify13')
...
//	var extension = require('../libs/html_extension')
	...
	transformer.execute(code).once('ready', function(output) {
		self.assertEquals("testextensions.htmlform", output, verify, 'output', 'verify')
		...
		transformer.execute(code).once('ready', function(output) {
			self.assertEquals("testextensions.htmlform2", output, verify, 'output', 'verify')
...
Tests.prototype.test_namespace1 = function() {
	...
	transformer.applyTemplate(input, {myfield: 'VARIABLE'}, null).once('ready', function(output) {
...
Tests.prototype.test_namespace2 = function() {
	...
	var verify = "Testing namespaces I am thing hello and another I am another thing hello, Var1 xyz VARIABLE Var2 VARIABLE abc, Block <<some text>> and [[some text]]"
	transformer.applyTemplate(input, {myfield: 'VARIABLE'}, null, {namespace: 'ext1'}).once('ready', function(output) {
...
Tests.prototype.test_namespace3 = function() {
	...
	var verify = "Testing namespaces I am thing hello and another I am another thing hello, Var1 xyz VARIABLE Var2 VARIABLE abc, Block <<some text>> and [[some text]]"
	transformer.applyTemplate(input, {myfield: 'VARIABLE'}, null, {namespace: 'ext2'}).once('ready', function(output) {
		self.assertEquals('namespace3.2', output, verify, 'output', 'verify')
		self.next()
	})
}

Tests.prototype.test_include0 = function() {
	var self = this
	var transformer = new Transformer
	var input = "Testing include {include name=inc0} ALL DONE";
	var verify = "Testing include Included Data\n ALL DONE"
	var options = {include_path: [Path.resolve(__dirname, 'data', 'elements')]}
	transformer.applyTemplate(input, {}, null, options).once('ready', function(output) {
		self.assertEquals('include0', output, verify, 'output', 'verify')
		self.next()
	})	
}
...
Tests.prototype.test_include1 = function() {
	...
	var options = {include_path: [Path.resolve(__dirname, 'data', 'elements')]}
	transformer.applyTemplate(input, {var1: 'VAR1', var2: 'VAR2', list: ['one', 'two','three']}, null, options).once('ready', function(output) {
...
Tests.prototype.test_include2 = function() {
	...
	var options = {include_path: [Path.resolve(__dirname, 'data', 'elements')]}
	transformer.applyTemplate(input, {var1: 'VAR1', var2: 'VAR2', list: ['one'], list2: ['alpha']}, null, options).once('ready', function(output) {
...
Tests.prototype.test_include3 = function() {
	...
	transformer.applyTemplate(input, {var1: "VAR1"}, null).once('ready', function(output) {
		var verify = 'Testing include namespace NS1 VAR1\n NS2 VAR1\n NSDEFAULT VAR1\n'
		self.assertEquals('test_include3', output, verify, 'output', 'verify')
		self.next()
	})
}

// Test a macro that uses event to trigger delayed response
Tests.prototype.test_async1 = function() {
	var self = this
	var transformer = new Transformer
	var input = "Testing async macro {ext1:async}"
	var verify = "Testing async macro THIS IS DELAYED RESPONSE"
	
	var MyExtension1 = function() {
	}
	MyExtension1.prototype.extend({
		macro_async: function(context) {
			var e = new Emitter
			setTimeout(function() {
				e.emit('ready', "THIS IS DELAYED RESPONSE")
			}, 1000)
			return e
		}
	})

	transformer.addExtension(new MyExtension1, 'ext1')
	transformer.applyTemplate(input).once('ready', function(output) {
		self.assertEquals('test_async1', output, verify, 'output', 'verify')
		self.next()
	})
}

// Test a macro that triggers delayed response without an event
// This is the most common use case for async macros
Tests.prototype.test_async2 = function() {
	var self = this
	var transformer = new Transformer
	var input = "Testing async macro 50 + 100 = {calc:sum value1=50 value2=100}. 300 + 600 = {calc:sum value1=300 value2=600}"
	var verify = "Testing async macro 50 + 100 = 150. 300 + 600 = 900"
	
	var MyExtension1 = function() {
	}
	MyExtension1.prototype.extend({
		macro_sum: function(context) {
			var dv = new DelayedValue
			setTimeout(function() {
				dv.value(parseInt(context.params.value1) + parseInt(context.params.value2))
			}, 1000)
			return dv
		}
	})

	transformer.addExtension(new MyExtension1, 'calc')
	transformer.applyTemplate(input).once('ready', function(output) {
		self.assertEquals('test_async2', output, verify, 'output', 'verify')
		self.next()
	})
}
